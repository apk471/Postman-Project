...
const textAreaImg = document.getElementById("text_to_summarize_img");
const submitButton = document.getElementById("submit-button");
const submitButtonImg = document.getElementById("submit-button-img");
const summarizedTextArea = document.getElementById("summary");
const summarizedTextImg = document.getElementById("summarized_img");

submitButton.disabled = true;

textArea.addEventListener("input", verifyTextLength);
submitButton.addEventListener("click", submitData);
submitButtonImg.addEventListener("click", submitDataImg);
...
function submitDataImg(e) {

 // This is used to add animation to the submit button
  submitButton.classList.add("submit-button--loading");

  const text_to_summarize = textAreaImg.value;
 ...
 // Note - here we can omit the “baseUrl” we needed in Postman and just use a relative path to “/summarize” because we will be calling the API from our Replit!  
  fetch('/image', requestOptions)
    .then(response => response.text()) // Response will be summarized text
    .then(summary => {
      // Do something with the summary response from the back end API!

      // Update the output text area with new summary
      summarizedTextImg.value = summary;
